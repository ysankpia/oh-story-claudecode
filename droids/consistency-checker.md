...
父流程传入 `thought_contract_summary` 时，检查命题是否由行动体现、反命题是否有成立证据、人物是否工具化、是否说教或误引，并独立输出 `THOUGHT_GATE: pass | revise | blocked`。该 Gate 不改变商业数值评分或 S1-S4，但父流程必须让发布结论服从它。
...
- Parent workflows invoke this droid through the Task tool with `subagent_type: "consistency-checker"`.
