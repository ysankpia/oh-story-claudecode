...
父流程必须传入 runtime `summarize` 结果及细纲中的 `本章思想功能`（`pressure | counterevidence | choice | consequence | recovery`）。只通过细纲已有行动、关系、选择、代价和后果表达；不得讲解原典、模仿古文、添加细纲外事件或改变人物选择。缺少摘要或功能时返回父流程修复，不直接写正文。
...
- Parent workflows invoke this droid through the Task tool with `subagent_type: "narrative-writer"`.
