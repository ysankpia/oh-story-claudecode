...
description: "小说写作资料研究 agent。接收研究查询，优先使用 CDP (agent-browser) 搜索并提取完整正文，\nWebSearch/webReader 作为兜底。输出带来源引用的结构化 Markdown 参考文件。\n被 story-long-write（Phase 4）、story-review、story skill 路由调用。"
model: inherit
tools: ["Read", "Glob", "Grep", "Execute", "Create", "Edit"]
---
...
- Parent workflows invoke this droid through the Task tool with `subagent_type: "story-researcher"`.
