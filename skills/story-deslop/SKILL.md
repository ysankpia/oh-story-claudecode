...
项目文件模式必须运行 `story-tao` bundled runtime 的 `ensure` 和 `summarize`，把思想契约中的核心对立、人物立场、意象和必要留白作为保护栏；只改表达，不改变人物选择、代价和后果。修复失败返回 `thought_contract_blocked` 并停止润色。无项目纯文本模式只生成临时 `provisional` 摘要，不落盘。
