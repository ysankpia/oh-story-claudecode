...
拆文必须调用 `story-tao` bundled runtime 的 `map-evidence`：先归纳作品实际命题、反命题、人物立场、关键选择、代价和后果，再映射恰好三张命题卡，写入 `拆文库/{书名}/思想/命题张力.md`。至少列三处小节或段落证据，并声明分析映射不证明原作者受老子影响；失败返回 `thought_contract_blocked`。
