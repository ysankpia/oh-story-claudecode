...
导入完成结构分析后必须读取 `拆文库/{书名}/思想/命题张力.md`，并运行 `story-tao` bundled runtime 的 `ensure`。长篇创建 `设定/思想命题.md` 与 `追踪/思想进展.md`，短篇创建根 `思想命题.md`；拆文思想产物缺失时从代表性正文补做证据映射。材料不足写 `provisional`，修复失败返回 `thought_contract_blocked` 并停止迁移，既有正文不回写。
