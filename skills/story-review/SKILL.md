...
1. **强制思想契约预检**：先调用 `story-tao` bundled runtime 的 `ensure`，再调用 `summarize` 生成 `thought_review_summary`。长篇读取 `设定/思想命题.md`，短篇读取项目根 `思想命题.md`；缺失时自动创建，旧 `confirmed` 自动迁移为 `active`。未知格式、非法卡片或章节越界返回 `thought_contract_blocked`，此时 `Thought Gate: blocked` 且禁止发布，不得降级为“忽略思想层”。
2. **确定请求模式**：解析用户输入中的 `full`、`lean`、`solo`；未指定时目标模式为 `full`。
3. **确认是否允许 spawn**：如果当前已经在子代理/Agent 内执行，不再递归 spawn，直接降级为 `solo`。
4. **检查核心 Agent 部署状态**（只检查当前运行端对应的目录）：
   - Claude Code 使用 `.claude/agents/`，Codex 使用 `.codex/agents/`，Droid 使用 `.factory/droids/`。
    - full 必需：Claude/Droid 为 `story-architect.md`、`character-designer.md`、`narrative-writer.md`、`consistency-checker.md`；Codex 为同名 `.toml`
    - lean 必需：Claude/Droid 为 `story-architect.md`、`consistency-checker.md`；Codex 为同名 `.toml`
    - 对每个必需 Agent 文件：
      ...
      - **Droid custom droid（`.factory/droids/`）**：读取 frontmatter，确认 `name:` 与 subagent_type 完全一致，且 `model: inherit`；frontmatter 缺失、不可解析或 name 不匹配时视为 malformed agent。
    - 如果 `.story-deployed` 存在且 `agents_version` 缺失、非整数或小于 `23`，视为 stale deployment；不要 spawn，降级 `solo`，建议用户重新运行 `/story-setup`。`agents_version` 大于 `23` 时也不 spawn：这表示当前 skill 比项目部署旧，降级 `solo` 并提示先更新 oh-story-claudecode，不要用 v23 重新部署。
...
3. `{项目根}/.factory/skills/{规范路径}`（Droid 项目内安装）
...
6. **思想命题摘要（必选）**：使用 Phase 0 runtime `summarize` 的结构化结果，检查核心命题、反命题、人物立场、当前检验、选择/代价/后果和表达禁区；不得直接读取全部命题卡。`active` 进入内容检查，`provisional` 至少为 `Thought Gate: revise`。
...
**story-explorer 预查询（可选）**。仅当 `Effective Mode` 仍为 `full`/`lean`、当前允许 spawn 且 Agent/Task 工具可用时，才可检查当前端 `.claude/agents/story-explorer.md`、`.codex/agents/story-explorer.toml` 或 `.factory/droids/story-explorer.md` 并 spawn `story-explorer` 预查设定摘要；`solo` 或子代理递归保护场景下不得 spawn，只能直接 Read/Grep。Prompt 示例：
...
使用 Agent/Task 工具并行调用（Codex 原生子代理使用 `agent_type`，Claude Code 与 Droid 使用 `subagent_type`；实际字段以当前 CLI 暴露的工具为准）。Droid 可将独立 reviewer Task 设为后台执行，但父流程必须通过 TaskOutput 收齐全部结果后才能进入裁决。每个 Agent 不继承父对话上下文，prompt 必须自包含项目路径、审查范围、文件路径、必要摘录、审查基准包摘要、Rubric Source 和统一 Findings Schema。

必须注入 `thought_review_summary`，要求 reviewer 另列 `THOUGHT_ADVICE`：检查命题是否由行动体现、反命题是否成立、人物是否工具化、是否说教或误引。思想问题不得转换成商业评分的 S1-S4 finding，但会共同决定独立的 `Thought Gate` 和发布结论。
...
3. **可选事实核查**：如果审查内容涉及需要验证的外部事实（历史年代、地理方位、职业细节等），只有在 `Effective Mode` 仍为 `full`/`lean`、当前不是子 Agent、Agent/Task 工具可用且当前端 `.claude/agents/story-researcher.md`、`.codex/agents/story-researcher.toml` 或 `.factory/droids/story-researcher.md` 已部署时，才可额外 spawn `story-researcher` 搜索验证；`solo`、missing/malformed/stale/spawn failed 降级或子代理递归保护场景下不得 spawn，只能在报告中标记“需人工事实核查”。
4. **分歧呈现**：如果 reviewer 间有冲突意见，明确呈现分歧让用户裁决；不要自动妥协。
5. 输出综合审查报告。报告必须列出实际模式、fallback 原因、使用的 rubric、Rubric Source、审查范围和证据不足项。
6. **思想完整性闸门（必选）**：合并 runtime 状态与 reviewer 的 `THOUGHT_ADVICE`，检查行动体现、反命题强度、人物工具化、说教/误引和读者契约冲突，输出 `Thought Gate: pass | revise | blocked`。商业数值评分与 S1-S4 严重级别保持独立、不计入思想问题；发布结论必须服从 Thought Gate：`blocked` 禁止发布，`revise` 标记待修订，只有 `pass` 可沿用商业结论。
...
Rubric: fanqie | qidian | zhihu | generic web-fiction
Rubric Source: file | embedded fallback
Thought Gate: pass | revise | blocked
审查范围: {章节/文件/批次}
...
## 思想完整性（固定闸门，商业数值评分独立）
{列行动体现、反方成立、人物工具化、说教/误引、契约冲突及发布约束}
...
6. 必须执行思想完整性检查并输出独立 `Thought Gate`；不计入 Findings、商业数值评分或 S1-S4，但发布结论必须服从 Gate。
