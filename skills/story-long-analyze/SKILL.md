...
拆文必须调用 `story-tao` bundled runtime 的 `map-evidence`：先归纳作品实际命题、反命题、人物立场、关键选择、代价、后果与反证，再映射恰好三张命题卡，写入 `拆文库/{书名}/思想/命题张力.md`。每项必须带章节证据，并声明分析映射不证明原作者受老子影响；证据无法验证时返回 `thought_contract_blocked`。
