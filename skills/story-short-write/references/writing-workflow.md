...
在创建 `设定.md` 或 `小节大纲.md` 前，调用 `story-tao` bundled runtime：

1. 以题材、平台、情绪目标、主角目标、核心冲突和现有证据执行 `ensure --mode short`。
2. 确认项目根 `思想命题.md` 为 `active | provisional`，且包含“初始信念 → 反例冲击 → 最终选择”。
3. `thought_contract_blocked` 时停止设计并报告修复原因；不得绕过、不得改成可选项。
4. 后续只调用 `summarize --mode short` 取得短摘要，不直接加载六十张卡或八十一章原典。
...
4. **编写 小节大纲.md**：每节 1 行，格式如下：
   `结构段/五段功能 | 思想步骤 | 主事件 | 子事件×3-5 | 情绪 | 人物/关系变化 | 因果/逻辑链 | 读者新获知什么 | 结尾承接/钩子 | 伏笔/物件 | 动静 | 对话密度 | 目标字数`
...
工作目录结构见 [Phase 2：构思核心框架](../SKILL.md#phase-2构思核心框架)。思想命题.md 是强制权威产物；设定.md 内容构成：核心框架 + 思想三步与人物立场 + 人设 + 反转铺垫 + 贯穿道具（含三次出现追踪表）+ 反派设计 + 对标摘要。小节大纲.md 每节 1 行，格式以 [设计任务（框架确定后执行）](../SKILL.md#设计任务框架确定后执行) 第 4 步为准。
