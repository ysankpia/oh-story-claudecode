...
题材、平台和情绪目标确定后，必须调用 `story-tao` bundled runtime 的 `ensure --mode short`，在项目根创建或校验 `思想命题.md`。短篇固定使用“初始信念 → 反例冲击 → 最终选择”三步检验：`设定.md` 保存三步与人物立场，`小节大纲.md` 为每节标记承载步骤，`正文.md` 只用行动、关系和后果呈现。写每节前调用 `summarize --mode short`；`thought_contract_blocked` 必须停止创作，不能跳过思想层或直接加载全部命题卡。
...
3. 确定揭露方式 → 同上
4. 编写 小节大纲.md（格式见 writing-workflow.md）：短篇只做轻量蓝图，每节包含结构段/五段功能、思想步骤（初始信念/反例冲击/最终选择）、人物/关系变化、因果/逻辑链、结尾承接/钩子，不套长篇完整章节蓝图。**标出付费点卡在哪一节末**（见 `submission-craft.md`「付费点」：卡脖子断点、付费点前每章 2-3 剧情点、付费点后每章多 1-2）；用反推法先想透付费点那一节，再倒排前后。每节可选一个任务卡点，但必须服务情绪升级、证据推进、关系撕裂、反转铺垫或反击动作；没有就不强补
...
- **思想摘要（必选）**：先调用 runtime `summarize --mode short`，只注入本节对应的三步位置、人物立场、选择/代价/后果和表达禁区；不得新增大纲外事件或改写人物最终选择。
