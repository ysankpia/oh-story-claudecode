...
生成前必须运行 `story-tao` bundled runtime 的 `ensure`，再运行 `summarize` 提取核心对立、关键意象、人物选择和结局回答，转为视觉方向。封面不得直接堆砌《道德经》经文、太极符号或泛化古风装饰；契约修复失败返回 `thought_contract_blocked` 并停止生成。
