...
>
> **强制思想内核**：每章先调用 `story-tao` runtime 的 `ensure` 和 `summarize`，只抽取当前人物立场、本章思想功能、选择/代价/后果、读者契约兼容与表达禁区，形成短 `thought_contract_summary`。文件缺失时自动创建，旧契约自动迁移；修复失败返回 `thought_contract_blocked` 并停止日更。不得把原典全文、候选卡或解释段落送入正文写手。
>
...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **思想命题摘要（必选）**：运行 `story-tao` bundled runtime 的 `ensure` 和 `summarize`，从 `设定/思想命题.md` + `追踪/思想进展.md` 提取当前人物立场、思想功能、选择/代价/后果和表达禁区，形成 `thought_contract_summary`。摘要只能解释细纲内选择的意义，**不得新增细纲外事件**、新角色、新反转或第二条哲学主线。冲突时保留原纲并标记 `thought_alignment_conflict`；修复失败返回 `thought_contract_blocked` 并停止本章。
   - **对标模块/节奏/题材卡/文风召回**：
   ...
   - **意图确认**：从细纲「目标情绪」字段确认本章情绪目标，综合状态筛选结果 + `selected_emotion_module` + `rhythm_reference` + `genre_prose_card` + **必选 `thought_contract_summary`** + 文风召回输出，用一句话写本章意图。材料不足只能保留 `provisional` 并显式列待补证据，不得省略思想项。其余情绪、节奏、主角代理权、阶段边界和细纲优先规则保持不变。
...
4. **推进思想状态（强制）**：正文、上下文和伏笔状态落盘后，调用 `story-tao` bundled runtime 的 `advance`，传入已完成章号、细纲既定的思想功能及正文中实际出现的立场/代价/后果。`advance` 必须幂等，且不得记录未来事件；失败时停止本章流程并报告 `thought_progress_update_failed`，不得进入下一章。
