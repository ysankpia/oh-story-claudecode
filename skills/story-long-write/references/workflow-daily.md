...
>
> **可选思想命题**：若 `设定/思想命题.md` 存在且 YAML 头为 `status: confirmed`，每章只抽取当前人物立场、本章对应的命题检验、选择/代价/后果、读者契约兼容与表达禁区，形成短 `thought_contract_summary`。文件缺失时完全跳过；未确认、损坏或来源无效时给出修复提示后忽略，不阻塞日更。不得把原典全文、候选卡或解释段落送入正文写手。
>
...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **思想命题摘要（可选）**：确认文件有效后，只提取本章相关四项形成 `thought_contract_summary`：① 当前人物立场；② 当前处于开篇/发展/高潮哪次命题检验；③ 既定选择、代价与后果；④ 读者契约兼容与禁止说教/误引/古风腔污染。摘要只能解释细纲内选择的意义，**不得新增细纲外事件**、新角色、新反转或第二条哲学主线。思想命题与用户要求、锁定细纲、读者契约或主角代理权冲突时，保留原纲并标记 `thought_alignment_conflict`；不得自动补纲或改纲
   - **对标模块/节奏/题材卡/文风召回**：
