...
> 运行环境兼容性：Claude Code、Codex 与 Droid 是内置适配目标。按当前端检查 `.claude/agents/{agent}.md`、`.codex/agents/{agent}.toml` 或 `.factory/droids/{agent}.md`；找不到或 registry 不可用时，直接 solo/direct 执行并报告 fallback。Droid 使用 Task 的 `subagent_type`，长任务可后台运行但必须由父流程收回结果并落盘。
...
story-architect 属于高层级结构设计 agent。轻量题材定位优先由主会话完成；只有涉及复杂世界观、多线结构、强反转工程或用户明确要求时，才调用 story-architect。确认选题方向后，如果当前端已部署 `.claude/agents/story-architect.md`、`.codex/agents/story-architect.toml` 或 `.factory/droids/story-architect.md`，可 spawn `story-architect`（Droid/Claude 使用 `subagent_type`，Codex 使用 `agent_type`）辅助题材分析和核心梗设计。如 agent 不可用，由主线程直接执行。
...
题材方向、平台、读者契约和书名目录确定后，必须自动调用 `story-tao` bundled runtime 的 `match` + `ensure`，在核心设定落盘前创建或修复 `设定/思想命题.md`，并创建 `追踪/思想进展.md`。不等待作者确认是否启用；作者只能覆盖主/辅命题。材料不足生成 `status: provisional`，无法修复才返回 `thought_contract_blocked` 并停止创作。

角色档案必须写初始立场、维护利益、底线、自我欺骗和立场改变条件；全书大纲承载最终回答，卷纲承载阶段检验，细纲每章标记 `pressure | counterevidence | choice | consequence | recovery` 之一。每章写完运行 `advance` 更新思想进展。

固定优先级：用户明确要求、锁定细纲和读者契约 > 思想命题 > 题材取舍与文风。思想命题需要新增细纲外事件或削弱主角代理权才能成立时，保留原纲并标记 `thought_alignment_conflict`，不得自动改纲。
...
- **设定/思想命题.md**（强制）：由 `story-tao` runtime 自动创建或迁移；主角、核心反方和关键配角必须持有可辩护的不同立场，不能把角色写成观点传声筒

> **多对标书时**：参 `references/cross-book-recall.md`，副对标 anchor 入「对标分析」表附录

#### Agent 调用：story-architect + character-designer

核心设定阶段，如果当前端已部署对应 agent（`.claude/agents/*.md`、`.codex/agents/*.toml` 或 `.factory/droids/*.md`），可 spawn 以下 agent 辅助：
...
- 思想命题检验：{无 confirmed 思想命题写“无”；否则写本卷承接开篇/发展/高潮哪次检验，以及选择/代价/后果；不得新增独立哲学支线}
- 核心事件：{一句话}
...
存在 confirmed `设定/思想命题.md` 时，再检查开篇、发展、高潮三次命题检验是否分别落入对应阶段，并且每次都有角色选择、代价和可见后果。思想检验必须依附现有核心冲突和剧情单元；若与读者契约、主角代理权或锁定结构冲突，记录 `thought_alignment_conflict` 并遵循原纲，不为满足命题另造剧情。
...
若当前端已部署 story-architect agent，可让它辅助：
...
| 设定/文风.md（自定义文风·优先级最高） | 本书 | 用户自写（当前主会话可代写）；导入/拆解不覆盖 | Phase 4 每章写作前：含实质内容则取代对标文风作权威风格基 |
...
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如果当前端已部署 `.claude/agents/story-explorer.md`、`.codex/agents/story-explorer.toml` 或 `.factory/droids/story-explorer.md`，可 spawn `story-explorer` 一次获取上下文）：
   ...
   - (15) `story-tao` runtime 的 `summarize` 结果（必选）— 只消费本章相关人物立场、思想功能、选择/代价/后果和表达禁区；契约缺失时由 `ensure` 创建，修复失败返回 `thought_contract_blocked` 并停止写作
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - **模块召回、题材卡与文风召回**：
     ...
     - **快捷路径**：项目已部署 story-explorer agent 时，可一次性召回文风/模块材料。
       - 只检查当前端：`.claude/agents/story-explorer.md`、`.codex/agents/story-explorer.toml` 或 `.factory/droids/story-explorer.md`。
...
4. **资料研究**（按需）：如果写作中遇到需要查证的外部事实（历史年代、地理方位、职业细节等），且当前端已部署 `.claude/agents/story-researcher.md`、`.codex/agents/story-researcher.toml` 或 `.factory/droids/story-researcher.md`，spawn `story-researcher` 搜索并输出到 `参考资料/` 目录。如 agent 不可用，由主线程直接执行。研究完成后再继续写作。
...
7. **正文执行**：
   - 先检查当前端 narrative-writer：`.claude/agents/narrative-writer.md`、`.codex/agents/narrative-writer.toml` 或 `.factory/droids/narrative-writer.md`。
