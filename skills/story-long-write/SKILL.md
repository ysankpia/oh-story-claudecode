...
用户明确要求“道德经内核 / 思想命题 / 用道德经深化小说”时，在题材方向、平台、读者契约和书名目录确定后调用 `story-tao`：由它固定推荐三个候选，等待作者确认一个主命题和最多一个辅命题，再创建 `设定/思想命题.md`。未确认前不得把候选写进核心设定或大纲。

项目已有 `设定/思想命题.md` 时只消费 `status: confirmed` 的文件。缺失、未确认、卡片来源无效或必填段落损坏时给出非阻断修复提示并忽略思想层，原写作流程保持不变。思想命题只校准人物立场、选择与后果，不自动改变文风，不要求正文引用原典。
...
- **设定/思想命题.md**（可选）：仅由已确认的 `story-tao` 流程创建；主角、核心反方和关键配角必须持有可辩护的不同立场，不能把角色写成观点传声筒
...
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如果项目已部署 story-explorer agent（优先检查 `.claude/agents/story-explorer.md` 是否存在；不存在时检查 `.codex/agents/`），可 spawn `Agent(subagent_type: "story-explorer", prompt: "项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章")` 一次获取上下文）：
   ...
   - (15) `设定/思想命题.md`（如存在且 `status: confirmed`）— 只抽取本章相关人物立场、命题检验、选择/代价/后果和表达禁区；未确认或损坏时提示修复并忽略，不阻塞写作
