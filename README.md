...
```
安装这个 skill https://github.com/ysankpia/oh-story-claudecode
```
...
>
> **Droid 用户：** clone 后可在仓库内直接运行 `droid`，它会从 `.factory/skills` 发现 14 个 skill、从 `.factory/droids` 发现 7 个 custom droid。也可先执行 `droid plugin marketplace add https://github.com/ysankpia/oh-story-claudecode`，再执行 `droid plugin install story@oh-story-claudecode --scope user`。进入小说项目后运行 `/story-setup` 并选择 `droid`，审核 `/hooks` 后新开 Droid 会话。
>
> 升级后如果项目里已经跑过 `/story-setup`，建议在项目根重跑一次 `/story-setup`，同步 hooks / agents / references。每版变更见 [CHANGELOG.md](CHANGELOG.md) 与 [Releases](https://github.com/ysankpia/oh-story-claudecode/releases)。
...
- **GitHub Discussions**：[提问 / 求助 / 分享用法](https://github.com/ysankpia/oh-story-claudecode/discussions)，方便检索。
